...
class T2V_dataset(Dataset):
    ...
    def define_frame_index(self, data):
        ...
        print(f"folder_anno: {folder_anno}")
        ...
        for sub_root, anno in tqdm(folder_anno):
            ...
            for index, i in enumerate(tqdm(sub_list)):
                print(f"Processing item {index}: {i['path']}")  # Debugging: Print each item being processed
                print(f"sub_root: {sub_root}")
                print(f"i['path']: {i['path']}")
                ...
                if path.endswith('.mp4'):
                    ...
                    #     print("continue because to long", i)
                    #     cnt_too_long += 1
                    #     continue
                    ...
                    #     cnt_too_short += 1
                    #     print("continue because too short", i)
                    #     continue
                    ...
                    #     cnt_too_short += 1
                    #     print("continue because too short 2", i)
                    #     continue
        ...
        print(f"new_cap_list length: {len(new_cap_list)}, sample_size length: {len(sample_size)}")
        print(f"new_cap_list: {new_cap_list}")
        print(f"sample_size: {sample_size}")
        #the problem is that here somehow the data is not found or something
    ...
    def get_actual_frame(self, fps, start_frame_idx, clip_total_frames, path, predefine_num_frames, predefine_frame_indice):
        ...
        # Allow all available frames instead of raising an error
        if len(frame_indices) < self.num_frames:
            print(f"Using available frames for video {path}: {len(frame_indices)} frames.")
            return frame_indices  # Return whatever frames are available
        ...
        # Find suitable end_frame_idx
        end_frame_idx = find_closest_y(len(frame_indices), vae_stride_t=self.ae_stride_t, model_ds_t=self.sp_size)
        if end_frame_idx == -1:
            print(f"Video {path} is too short to encode. Using available frames.")
            return frame_indices  # Return whatever is available

        frame_indices = frame_indices[:end_frame_idx]
        if predefine_num_frames != len(frame_indices):
            print(f"Warning: predefine_num_frames ({predefine_num_frames}) does not match actual ({len(frame_indices)}).")
